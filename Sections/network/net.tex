...
\begin{Def}[Uniform Resource Identifier (URI) \& URLs]
    ...
    A \textbf{Uniform Resource Locator (URL)} is a type of URI that specifies the resource's location.
    A \textbf{Uniform Resource Name (URN)} is another type of URI that identifies a resource by name in a specific namespace.
    Designed to unique and present even when its resource is not available (e.g., a book isbn, urn:isbn:0451450523).
\end{Def}

\begin{figure}[h!]
    \hspace{-2.5em}
    \includegraphics[width=1.1\textwidth]{Sections/network/httpheader.png}
