...
\begin{Def}[IPv4 Packet Header]
    ...
    The header sits \textit{ahead} of the \textbf{payload} (data) in the packet.\\
    Eg., [IP Header (20–60 bytes), TCP Header (20–60 bytes), Payload (Variable)]. in a stream of bits. \hfill \cite{rfc791}
\end{Def}
\begin{figure}[h!]
    \centering
    \resizebox{1\textwidth}{!}{
...
\begin{Def}[Internet Control Message Protocol (ICMP)]
    ...
    \textbf{Internet Control Message Protocol (ICMP)} is a supporting protocol used by network devices to communicate error messages and operational information.
    \begin{itemize}
        ...
        \item \textbf{Fragmentation Needed}: Informs the sender that the packet is too large for a network segment and cannot be fragmented.
        \item \textbf{Echo Request and Reply (Ping)}: Used to test connectivity and measure round-trip time between devices. \hfill \cite{rfc792}
    \end{itemize}
\end{Def}
...
\newpage
\noindent
The following solution was implemented to address the exhaustion of IPv4 addresses:
